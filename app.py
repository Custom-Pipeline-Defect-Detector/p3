...
def diseaseAnalysis():
    ...
    if modelType == "YoloX":
        ...
        weight_override = request.form.get("efficientnet_yolov8_weights")
        model = load_model('efficientnet-yolov8', weights_path=get_efficientnet_yolov8_weights(weight_override))
...
def detect():
    ...
    for upload_file_path in upload_files_path:
        if IMAGE_FLAG:
            record_identify_from_file = True
            if y57_c:
                print("配置yolov5后")
                print(y57_c)
                ...
                weight_override = result_model.get("efficientnet_yolov8_weights")
                model = load_model('efficientnet-yolov8', weights_path=get_efficientnet_yolov8_weights(weight_override))
                image = Image.open(upload_file_path)
                result_image, detections = detect_objects(model, image)
                output_dir = './static/output/efficientnet_yolov8'
                ...
                if result_model["model_type"] == "YOLOv5":
                    ...
                    run_yolov_5_7(source=upload_file_path, yolo_version=7)
            if record_identify_from_file:
                with open('./inference/output/identify_image.txt', "r+", encoding='UTF-8') as f:
                    identify_path = f.readline().strip()
                    if identify_path:
                        identify_images.append(identify_path)
                    f.truncate(0)
        elif VIDEO_FLAG:
            # 调整关键帧提取过程的参数
            if kf_c:
...
# Ensure default empty structures for neo4j visualization data and create the file if missing
neo4j_data = {'data': [], 'links': []}
os.makedirs("static/owl_json", exist_ok=True)
with open("static/owl_json/owl_nodes_relationships.json", "w", encoding='utf-8') as f:
    json.dump(neo4j_data, f)

@app.route('/neo4j_owl')
def neo4j_owl():
    # Read the JSON file safely and fall back to empty data if the file is missing or invalid
    try:
        with open("static/owl_json/owl_nodes_relationships.json", "r", encoding='utf-8') as f:
            neo4j_data_local = json.load(f)
    except (FileNotFoundError, json.JSONDecodeError):
        neo4j_data_local = {'data': [], 'links': []}
    return render_template("neo4j_owl.html", neo4j_data=neo4j_data_local)
